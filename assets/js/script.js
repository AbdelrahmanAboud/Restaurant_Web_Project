...
// Helper: Load all include-html and then run callback
function loadAllIncludes(callback) {
  ...
  includes.forEach(function(el) {
    ...
    if (file) {
      fetch(file)
        ...
        .catch(error => {
          el.innerHTML = `<div style='color:red;'>Could not load ${file}</div>`;
          loaded++;
          if (loaded === includes.length) {
            callback();
          }
...
function setupEventListeners() {
  ...
  function searchMenu() {
    if (!menuSearchInput || !menuSearchMsg) return;
  ...
  // Hide header (navbar) on scroll down, show on scroll up
  let lastScrollPos = 0;
  const header = document.querySelector('[data-header]');
  window.addEventListener('scroll', function () {
    if (!header) return;
    if (window.scrollY > lastScrollPos && window.scrollY > 50) {
      header.classList.add('hide');
    } else {
      header.classList.remove('hide');
    }
  ...
  // ربط زر المينيو بعد تحميل الـ navbar
  const navbar = document.querySelector('[data-navbar]');
  const navTogglers = document.querySelectorAll('[data-nav-toggler]');
  const overlay = document.querySelector('[data-overlay]');
  const toggleNavbar = function () {
...
document.addEventListener('DOMContentLoaded', function() {
  loadAllIncludes(setupEventListeners);
});
